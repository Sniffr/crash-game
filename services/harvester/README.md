...
Scrapes upcoming sports odds with [OddsHarvester](https://github.com/jordantete/OddsHarvester),
normalizes them into the **Simulate fixtures feed**, and uploads that feed to S3
...
```
OddsHarvester ──scrape──▶ normalize.py ──▶ fixtures feed ──▶ S3 (simulate/fixtures/latest.json)
                                                                     │
...
across all bookmakers OddsHarvester returned (FullTime period).
...
| Var | Default | Meaning |
| --- | --- | --- |
| `HARVEST_SPORT` | `football` | OddsHarvester sport |
| `HARVEST_MARKETS` | `1x2` | market token(s) to scrape |
...
| `HARVEST_TIMEOUT_SEC` | `900` | per-date scrape timeout |
| `S3_ENDPOINT` | — | e.g. `https://eu2.contabostorage.com` |
| `S3_REGION` | `eu2` | S3 region |
| `S3_BUCKET` | — | e.g. `crash` |
| `S3_ACCESS_KEY` / `S3_SECRET_KEY` | — | credentials |

If a scrape yields no fixtures (network down, OddsHarvester missing), the run
